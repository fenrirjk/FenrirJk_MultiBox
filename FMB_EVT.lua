...
function f_FMB_EVT_RemoteScript(msg)
    ...
    if l_nbArgs ~= 2 then
        f_FMB_UTL_Debug("f_FMB_EVT_RemoteScript: Format error")
    end
...
function f_FMB_EVT_OnEvent()
    ...
    if (event == "CHAT_MSG_ADDON") and (arg1 == "FenrirJk's multiboxing canal RemoteScript") then
        ...
        if l_nbArgs ~= 2 then
            f_FMB_UTL_Debug("f_FMB_EVT_OnEvent: Format error: " .. arg2)
        end

        if l_args[1] == "PARTY" or l_args[1] == UnitName("PLAYER") then
            f_FMB_UTL_Debug("Running script: " .. l_args[2])
            RunScript(l_args[2])
        end
    end
    ...
    if (event == "CHAT_MSG_SPELL_SELF_DAMAGE") then
        ...
        if (g_FMB_SPL_CurrentSpell ~= nil) then
            ...
            if (l_currentSpell ~= nil) and (strfind(arg1, l_currentSpell)) and (GetTime() < g_FMB_SPL_NextCastTime) then
                f_FMB_UTL_Debug("f_FMB_EVT_OnEvent: Fixing cast time of " .. g_FMB_SPL_CurrentSpell .. " to: " .. g_FMB_PlayerSpells[g_FMB_SPL_CurrentSpell].castTime - (g_FMB_SPL_NextCastTime - GetTime())/2)
